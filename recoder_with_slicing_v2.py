...
class PipelineData:
    def __init__(self, pipeline, src, main_loop, rec_loc_gen, probe_id):
...
def bus_callback(bus, message, pdata: PipelineData):
    message_type = message.type
    if message_type == Gst.MessageType.ELEMENT:
        msg_struct = message.get_structure()
        if msg_struct.get_name() == "GstBinForwarded":
            fwd_msg = msg_struct.get_value("message")
            if fwd_msg.type == Gst.MessageType.EOS:
                enc = pdata.pipeline.get_by_name("encoder_bin")
                enc.set_state(Gst.State.NULL)
                enc.get_state(Gst.CLOCK_TIME_NONE)
                pdata.pipeline.remove(enc)
                queue = pdata.pipeline.get_by_name('queue0')
                ...
                queue.get_static_pad("src").remove_probe(pdata.probe_id)
...
def rollover_cb(pdata: PipelineData):
    ...
    probe_id = queue_src_pad.add_probe(Gst.PadProbeType.BLOCK_DOWNSTREAM, probe_cb, pdata)
    pdata.probe_id = probe_id
...
def create_encoding_bin(location):
    ...
    enc_bin.set_property("message-forward", True)
...
def main():
    ...
    loop = GLib.MainLoop()
    pdata = PipelineData(pipeline, src, loop, rec_loc_gen, None)
